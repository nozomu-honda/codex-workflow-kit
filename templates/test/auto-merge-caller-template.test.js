...
const TEMPLATE_FILE = new URL('../workflows/reviewed-pr-auto-merge-events.yml', import.meta.url);
const REUSABLE_WORKFLOW_USES = 'nozomu-honda/codex-workflow-kit/.github/workflows/auto-merge-plan.yml@REPLACE_WITH_TAG_OR_40_CHAR_COMMIT_SHA';
...
test('auto-merge caller templateは必要な実イベントと手動dispatchだけを受ける', async () => {
  const { workflow } = await readTemplate();

  assert.deepEqual(Object.keys(workflow.on).sort(), EXPECTED_EVENTS.toSorted());
  assert.deepEqual(workflow.on.workflow_run.types, ['completed']);
  assert.deepEqual(workflow.on.check_suite.types, ['completed']);
  ...
  assert.deepEqual(workflow.on.pull_request.types, ['closed', 'ready_for_review', 'synchronize']);
...
test('callerは薄いjob-level reusable workflow呼び出しだけを持つ', async () => {
  ...
  const job = jobs['auto-merge-plan'];

  assert.deepEqual(Object.keys(jobs), ['auto-merge-plan']);
...
test('callerはpayload、導入先Variables、固定refだけを渡す', async () => {
  const { workflow } = await readTemplate();
  const withInputs = workflow.jobs['auto-merge-plan'].with;
  ...
  assert.deepEqual(withInputs, {
    ...
    sha: '${{ github.sha }}',
    'dry-run': true,
    'repository-config-json': "${{ vars.CHATGPT_AUTOMATION_AUTO_MERGE_CONFIG_JSON || '{}' }}",
    'existing-dedupe-keys': "${{ vars.CHATGPT_AUTOMATION_AUTO_MERGE_DEDUPE_KEYS || '' }}",
    'last-planned-at': "${{ vars.CHATGPT_AUTOMATION_AUTO_MERGE_LAST_PLANNED_AT || '' }}",
...
test('reusable workflow refとkit-refは同じ固定refへ置換する契約にする', async () => {
  const { workflow } = await readTemplate();
  const job = workflow.jobs['auto-merge-plan'];
