...
```yaml
- uses: nozomu-honda/codex-workflow-kit/actions/validate-config@03d54075f77034124b0b0982200b0d44059bed8a
  with:
    config-file: .github/chatgpt-automation.yml
    dry-run: 'true'
```

直接Actionを呼ぶ場合も、`master` / `main`、branch名、短縮SHA、tag参照ではなく、レビュー済みの40桁commit SHAへ固定します。通常の導入では、`templates/workflows/validate-config.yml` からreusable workflowを呼び出してください。
