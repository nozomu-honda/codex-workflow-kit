...
test('kit-ref検証は40桁SHAまたは完全version tagだけを許可する', async () => {
  ...
  assert.equal(isAllowedKitRef('REPLACE_WITH_TAG_OR_40_CHAR_COMMIT_SHA'), false);
  assert.equal(source.includes('REPLACE_WITH_TAG_OR_40_CHAR_COMMIT_SHA'), false);
});
...
function isAllowedKitRef(ref) {
  return /^[a-f0-9]{40}$/i.test(ref) || /^v\d+\.\d+\.\d+$/.test(ref);
}
