...
const WORKFLOW_FILE = new URL('../../.github/workflows/auto-merge-plan.yml', import.meta.url);
...
test('auto-merge reusable workflowはworkflow_callのみを入口にする', async () => {
  const { workflow } = await readWorkflow();

  assert.deepEqual(Object.keys(workflow.on), ['workflow_call']);
});

test('workflow_call inputs / outputsがauto-merge plan契約どおり', async () => {
  const { workflow } = await readWorkflow();
  const call = workflow.on.workflow_call;

  assert.deepEqual(Object.keys(call.inputs).sort(), REQUIRED_INPUTS.toSorted());
  assert.deepEqual(Object.keys(call.outputs).sort(), AUTO_MERGE_OUTPUT_NAMES.toSorted());
...
test('Issue #23 normalizerを呼び出し、plan scriptだけでoutputsを生成する', async () => {
  const { workflow } = await readWorkflow();
  const normalize = workflow.jobs['normalize-event'];
  const planner = workflow.jobs['auto-merge-plan'];
  const step = planner.steps.find((entry) => entry.id === 'auto-merge-plan');
...
test('共有kitを固定refでcheckoutし、kit-ref検証を持つ', async () => {
  const { workflow } = await readWorkflow();
  const steps = workflow.jobs['auto-merge-plan'].steps;
