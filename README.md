...
`scripts/audit-repository-protection.mjs` には、consumer repositoryのdefault branchに設定されたBranch protection、Ruleset、required checks、required reviews、bypass条件、merge settingsをGitHub API readだけで監査するCLIを置いています。設定変更、Check Run作成、PR / Issue / comment / label操作、Secret操作は行いません。標準 `github.token` を使うworkflowはread-only診断です。live auditを進める場合は手動で用意したAdministration read相当のexternal read tokenをCLIへ明示しますが、Ruleset `bypass_actors` は権限により省略されることがあり、省略時は `ruleset_bypass_visibility_unknown` でmanual reviewに回します。詳細は `docs/github-automation/protection-audit.md` を参照してください。

`scripts/audit-live-consumer.mjs` には、実consumer repositoryを変更せずにGitHub REST APIのGETだけで導入状態を監査するread-only CLIを置いています。default branch上のconfig / caller workflow / fixed SHA / permissions / triggers / Secret-like構成をsanitized reportへまとめます。詳細は `docs/github-automation/live-consumer-audit.md` を参照してください。
...
`.github/workflows/auto-merge-dry-run-executor.yml` には、auto-merge plan、review evidence、live consumer audit、repository protection audit、CI/check、changed files、write command validationを統合するdry-run executorを置いています。eligibleでも `DisabledGitHubWriteAdapter` が `write_disabled` で拒否し、GitHub API write、merge、auto-merge有効化、comment、label、deploy、release、tag作成は行いません。詳細は `docs/github-automation/auto-merge-dry-run-executor.md` を参照してください。

`.github/workflows/main-follow-up-plan.yml` には、default branchが進んだあとにopen PRを `up-to-date`、safeなbranch update候補、Codex follow-up候補、manual review required、ineligibleへ分類するreusable workflowを置いています。`plans_json`、`update_candidate_count`、`codex_follow_up_candidate_count` などのplan outputsを返しますが、PR branch update API、Codex起動、Queue Issue更新、コメント投稿、ラベル操作は行いません。導入先caller templateは `templates/workflows/main-follow-up-events.yml`、詳細は `docs/github-automation/main-follow-up.md` を参照してください。

`packages/chatgpt-automation-core/src/github-write/` には、既存plan outputsから将来のwrite command候補を作る境界を置いています。既定の `DisabledGitHubWriteAdapter` はvalid commandも `write_disabled` で拒否し、GitHub API writeやnetworkを行いません。trusted actor、plan source、command鮮度はworkflow境界から渡すvalidation contextでfail closedに検証し、command payloadの自己申告やgeneric snapshotではvalidになりません。テスト専用の `FakeGitHubWriteAdapter` はfixture allowance内でcommandを記録するだけで、実writeは行いません。詳細は `docs/github-automation/github-write-adapter.md` を参照してください。

`fixtures/auto-merge-regressions/` には、Reviewed PR auto-merge planの安全性回帰を完全offlineで再現するscenario fixtureとsnapshotを置いています。`scripts/replay-auto-merge-scenarios.mjs` はscenarioをauto-merge dry-run executorの実入力契約へ変換し、review evidence不足、stale review、CI不足、consumer / protection audit不足、dangerous diff、report freshness、command freshnessなどをdeterministicに検証します。GitHub API write、network、Secret、実consumer変更は行いません。詳細は `docs/github-automation/auto-merge-regressions.md` を参照してください。
