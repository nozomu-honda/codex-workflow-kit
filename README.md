...
`fixtures/auto-merge-regressions/` には、Reviewed PR auto-merge planの安全性回帰を完全offlineで再現するscenario fixtureとsnapshotを置いています。`scripts/replay-auto-merge-scenarios.mjs` は既存auto-merge planとGitHub write command境界を呼び出し、review evidence不足、stale review、CI不足、consumer / protection audit不足、dangerous diff、command freshnessなどをdeterministicに検証します。GitHub API write、network、Secret、実consumer変更は行いません。詳細は `docs/github-automation/auto-merge-regressions.md` を参照してください。
