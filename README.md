...
`scripts/audit-repository-protection.mjs` には、consumer repositoryのdefault branchに設定されたBranch protection、Ruleset、required checks、required reviews、bypass条件、merge settingsをGitHub API readだけで監査するCLIを置いています。設定変更、Check Run作成、PR / Issue / comment / label操作、Secret操作は行いません。詳細は `docs/github-automation/protection-audit.md` を参照してください。
