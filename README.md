...
`fixtures/github-events/` には、後続のreview routing、auto merge、main follow-upで共通利用するGitHub event payload fixtureを置いています。`issue_comment`、`pull_request`、`pull_request_review`、`pull_request_review_comment`、`workflow_run`、`push` をdeterministicに生成でき、same-repository、fork、merged、draft、CI success/failure、unknown actor、GitHub Actions bot、ChatGPT bot、invalid/malformed payloadをbuilder形式で扱えます。fixtureは完全offlineで、実repository、実URL、実メール、実SHA、Secret-like値を含めません。
