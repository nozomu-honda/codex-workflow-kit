...
- `issue_comment` がPR上のコメントで、PR head repositoryをpayloadだけで検証できない場合はfail closedにする
- `workflow_run`、`check_suite`、`check_run` のworkflow名、conclusion、head SHAを正規化する
- `push` の対象branchを正規化し、default branch以外をeligibleにしない
- `workflow_dispatch` の手動PR番号を正規化する
...
`workflow_dispatch` はreview routingなどの手動確認callerから再利用できる正規化対象です。Issue #23の `templates/workflows/chatgpt-automation-events.yml` には含めず、Issue #24のreview routing templateで使います。

Issue #25のauto-merge plan templateでは、CI/check完了を扱うため `check_suite` と `check_run` も正規化対象として再利用します。

Issue #26のmain follow-up planでは、`workflow_dispatch` をPR番号なしの全open PR scanとして再利用します。この場合だけ `requested-capability: main-follow-up-plan` によりPR番号なしdispatchをeligibleにします。
...
- `requested-capability`。通常は `normalize-only`、main follow-up planでは `main-follow-up-plan`
...
| `workflow_name` | `workflow_run` / `check_suite` / `check_run` のworkflowまたはcheck名。なければ空文字列 |
| `workflow_conclusion` | `workflow_run` / `check_suite` / `check_run` のconclusion。なければ空文字列 |
| `dry_run` | 実効dry-run |
...
- repositoryが一致しない
- fork / external PR
- PR上の `issue_comment` でPR provenanceを検証できない
- 必須のissue / PR番号がない
- 想定外action
- `workflow_run` / `check_suite` / `check_run` が `success` 以外
- `pull_request.closed` がmergedではない
- `push` がdefault branch以外
- `workflow_dispatch` に手動PR番号がない。ただし `requested-capability: main-follow-up-plan` の全PR scanは除く
- `dry-run` がbooleanとして解釈できない
- `permission-mode` が `read-only` 以外
- `requested-capability` が `normalize-only` / `main-follow-up-plan` 以外
...
Issue #24以降でreview routingやwrite処理を追加する場合も、Issue #23の `eligible`、fork/same-repository判定、dry-run、Secret境界を前提にします。
...
- ChatGPT review routing本体。Issue #24では `docs/github-automation/review-routing.md` の範囲でrouting plan生成までを追加し、write処理は追加しません。
- Reviewed PR auto-merge plan。Issue #25では `docs/github-automation/auto-merge.md` の範囲でplan生成までを追加し、write処理は追加しません。
