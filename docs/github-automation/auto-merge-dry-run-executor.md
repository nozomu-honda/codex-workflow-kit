...
各reportの `checkedAt` は `executionContext.now` 以前である必要があります。ただしGitHub Actions job間やAPI producer間の微小な時計差を吸収するため、executorは `60_000ms` の固定clock skewだけを許容します。この値は無制限に広げず、設定入力から変更しません。`executionContext.now + 60_000ms` を1msでも超える `checkedAt` / `reviewedAt` は `report_from_future` でfail closedします。
