...
- Secret-like keyと具体値、`${{ secrets.* }}`、`${{ secrets['NAME'] }}`、`${{ github.token }}`、`${{ github['token'] }}`、wrapper関数内のSecret/runtime token転送をfail closedにし、reportへ値、Secret名、式全文を出さない
...
- stepsが `actions/checkout@34e114876b0b11c390a56381ad16ebd13914f8d5` と `actions/validate-config` 呼び出しだけで、`run` を持たない
...
- refは40桁commit SHA、または明確な置換プレースホルダーにする
...
Release readinessは、fixed SHA rolloutのためのread-only gateです。

確認対象:

- release manifestがschemaVersion 1でvalid
- releaseCommitSha / previousReleaseCommitSha / rollbackCommitShaが40桁小文字commit SHA
- version tagは人間向け識別子であり、consumer実行参照には使われない
- `.github/workflows`、`reusable-workflows`、`templates/workflows`、docs、fixturesの `uses:` が固定SHA policyに従う
- `REPLACE_WITH_40_CHAR_COMMIT_SHA` はtemplate内だけに残る
- `npm run check:action-dist` が成功する
- `CHANGELOG.md` とmanifestのchanged capabilityが整合する
- consumer inventoryからplan-onlyの更新計画が作れる
- mixed refs、mutable refs、downgrade、breaking change、path traversalはfail closed
...
この確認はGitHub API write、tag作成、release作成、npm publish、consumer push、deployを行いません。
