...
標準 `github.token` ではAdministration readを確認できないため、workflowは完全監査済みの `ready=true` にはしません。`administration_read_token_required` でmanual setupを要求します。完全なlive auditには、利用者が手動で用意したAdministration read権限のGitHub App installation tokenまたはfine-grained PAT等をローカルCLIへ渡し、`--token-source external-read-token` を明示します。
