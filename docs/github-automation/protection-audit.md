...
- `github-token`: 標準 `github.token` 相当。Administration readを確認できないため、監査完了として `ready=true` にはしません。`administration_read_token_required` でmanual setupを要求します。
- `external-read-token`: 利用者が手動で用意したAdministration read相当のGitHub App installation token、またはfine-grained PAT等を想定します。Repository metadata、default branch/SHA、Branch protection、Ruleset一覧/detailのread-visible部分、required checks/reviews、enforcement、conditions、rule types、merge settingsの確認に使います。ただしtoken種別の自己申告だけで安全扱いせず、実API 403、pagination未完了、Ruleset bypass可視性不明は引き続きfail closedです。
...
workflowは標準 `github.token` をCLIへ `--token-source github-token` として渡します。標準 `github.token` はAdministration readを付与できないため、このworkflowはread-only診断・権限確認用途であり、監査完了として `ready=true` にはしません。

live auditを進める場合は、利用者が手動で用意したAdministration read権限を持つGitHub App installation token、またはfine-grained PAT等をローカル環境で使い、CLIへ `--token-source external-read-token` を渡します。ただしGitHub Ruleset APIでは、呼出元にRuleset write相当の可視性がない場合に `bypass_actors` が省略されることがあります。Administration read相当tokenだけではbypass actorの完全性を保証できないため、`bypass_actors` が省略されたRulesetが1件でもあれば `ruleset_bypass_visibility_unknown` で `ready=false` とし、manual reviewへ回します。このPRではSecret input、`secrets: inherit`、GitHub AppやPATの作成、Secret登録、write権限追加は行いません。権限追加前には手動レビューが必要です。
...
GitHub Ruleset detail APIでは、呼出元に十分な権限がない場合に `bypass_actors` が省略されることがあります。特にAdministration read相当のcredentialでも、Rulesetごとのwrite権限がなければbypass actorの完全な監査を保証できません。監査では次を区別します。
