...
実write処理へ進む前に、consumer repositoryのBranch protection / Ruleset / required checks / required reviews / bypass条件は [Repository protection audit](protection-audit.md) でread-only監査します。`Review evidence gate` と `CI` がrequired checkになっていること、conversation resolutionとstale approval dismissalが有効であること、bypass actorに自動化actorや広いteamがいないことを確認します。
...
2. planner job最初のstepでUTCのrun開始時刻を生成する。この値はcaller inputやevent payloadから受け取らない。
3. `scripts/plan-auto-merge.mjs` を実行し、run開始時刻を `runStartedAt` として渡すとともに、必要なPR情報、changed files、reviews、review threads、workflow runs、check runs、commit statuses、repository settings、compare結果、actor権限をGitHub API readで補完する。
4. `packages/chatgpt-automation-core/src/auto-merge/` のpure logicでauto-merge planを作る。
5. 後続workflowが読めるoutputsを公開する。
...
- 同一comment / review本文に `approved` markerと `changes_requested` markerが両方ある場合、またはmarkerとGitHub review state / status headingが競合する場合は、approvalではなく `changes_requested` 相当としてfail closedにする。fenced code block内markerとreview request comment除外は維持する。
- stale headのmarker、actorなしmarker、timestampが不正なmarkerはapprovalとして採用しない。不正timestampを含むcurrent-head markerは安全側に倒してblockする。
- human approvalは `autoMerge.trustedReviewers` に含まれるreviewerだけを対象にし、reviewer loginごとのcurrent-head approvalを1件だけ数える。
- 同一reviewerの複数 `APPROVED` は重複カウントしない。
- あるreviewerの `CHANGES_REQUESTED` はhead更新後も未解消として保持し、別reviewerの `APPROVED`、同reviewerの `COMMENTED`、同reviewerのstale-head `APPROVED` では解除しない。同じreviewerが現在headへ後続 `APPROVED` を出した場合だけ解除される。
- reviewerの最新reviewが `COMMENTED` でも、それ以前に未解消の `CHANGES_REQUESTED` があればblockする。
- stale headへの `APPROVED` は現在のapprovalとして数えず、未解消 `CHANGES_REQUESTED` の解除にも使わない。
- `DISMISSED` はapprovalや変更要求解除として扱わず、`dismissed_review` としてblockする。
- review timestampが不正、空、または順序を確定できない場合はfail closedにする。同一timestampで状態が競合する場合は `CHANGES_REQUESTED`、`DISMISSED`、`APPROVED`、`COMMENTED` の順に安全側の状態を優先する。
- 同じGitHub review sourceにChatGPT markerと `APPROVED` review stateが同居する場合は、ChatGPT reviewとしてのみ扱い、human approvalへ二重計上しない。
- `pull_request_review` が信頼済みChatGPT approvalをtriggerした場合、trigger review ID、actor、current head SHAをAPI取得結果と照合し、review timestampがplanner job内で生成したrun開始秒より前に確定できる場合だけ採用する。run開始後、同一秒、run開始時刻の欠落・空文字・不正値はfail closedにする。
- run開始時刻はworkflow runtimeが所有し、caller input、`workflow_run.run_started_at`、その他のevent payload値では代用しない。
...
Issue #37では、auto-merge planから将来write workflow向けのcommand候補を作るpure converterを追加します。

- `eligible=true` かつ `should_enable_auto_merge=true` の場合だけ `enable-auto-merge` command候補を作る
- `eligible=true` かつ `should_merge=true` の場合だけ `merge-pull-request` command候補を作る
- `skip_reason` があるplan、manual review相当、不明状態ではcommandを作らない
...
この境界は [github-write-adapter.md](github-write-adapter.md) を正とします。実際のauto-merge有効化、merge queue投入、merge API呼び出しはこのIssueでもIssue #37でも行いません。

## regression replay

Issue #42では、auto-merge planの安全性回帰scenarioを [Auto-merge regression replay](auto-merge-regressions.md) として固定します。

このsuiteは完全offlineで、sanitized fixtureをauto-merge dry-run executorの実入力契約へ変換し、`executeAutoMergeDryRun()` へ通します。PR #130相当のreview evidence不足、stale review、current-head判定、CI不足、consumer / protection audit不足、dangerous diff、duplicate / cooldown、report freshness、command freshness、成功候補の `write_disabled` をsnapshotで確認します。

このsuiteが成功しても、導入先Rulesetやrequired checksが実際に正しく設定されていること、GitHub UIでmerge可能であること、dry-run executorが実consumer上で動くことを確認した扱いにはしません。実consumer / Ruleset確認はRepository protection audit、Live consumer audit、または導入先Issueで別途実施します。
