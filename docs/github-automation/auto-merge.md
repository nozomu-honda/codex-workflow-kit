...
- ChatGPT review approved markerが最新headに対して有効
- 最新 `changes_requested` がない
- required approval数を満たす
- 未解決review threadがない
- requested reviewerが残っていない
...
- `<!-- chatgpt-review: approved -->` が現在headに紐づく場合だけChatGPT approvalとして扱う。
- `<!-- chatgpt-review: changes_requested -->` またはGitHub reviewの `CHANGES_REQUESTED` が最新なら停止する。
