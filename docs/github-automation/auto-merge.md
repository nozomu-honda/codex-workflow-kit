...
このsuiteは完全offlineで、sanitized fixtureを既存の `createAutoMergePlan()` とGitHub write command境界へ通します。PR #130相当のreview evidence不足、stale review、current-head判定、CI不足、consumer / protection audit不足、dangerous diff、duplicate / cooldown、command freshness、成功候補の `write_disabled` をsnapshotで確認します。
