...
`packages/chatgpt-automation-core/src/release-readiness/` と `scripts/plan-release.mjs` は、共通キットをconsumerへ安全に追従するためのread-only release readiness layerです。

- consumerの実行参照はレビュー済み40桁commit SHAを正本にする
- version tagは人間向けrelease識別子として扱い、consumer更新先にはしない
- release manifest、CHANGELOG、source/dist、fixed ref、consumer inventoryを検証する
- consumer updateは計画だけを生成し、PR作成、push、tag、release、npm publish、deployは行わない
- 検証不能、不明、source/dist不一致、ref混在、breaking change、path traversalはfail closedにする
...
- `actions/checkout@34e114876b0b11c390a56381ad16ebd13914f8d5` でcaller repositoryをcheckoutする
...
- inputsはevent payload、repository情報、導入先config JSON、dedupe/cooldown情報、`kit-ref`
- run開始時刻はcaller inputにせず、planner job最初のstepでUTC timestampを生成して `scripts/plan-auto-merge.mjs` からcoreの `runStartedAt` へ渡す
- `.github/workflows/normalize-event.yml` を先に呼び、正規化eventが不適格ならfail closedにする
...
`.github/workflows/auto-merge-dry-run-executor.yml` は、auto-merge plan以降の最終dry-run decisionを作るreusable workflowです。

- inputsはauto-merge plan、review evidence report、consumer audit report、protection audit report、PR snapshot、checks、changed files、execution context JSON
- outputsは `eligible`、`command-created`、`adapter-accepted`、`executed`、`reason-codes-json`、`result-json`
- workflow / job permissionsは `contents: read`、`pull-requests: read`、`actions: read`、`checks: read`、`statuses: read`
- Secret input、`secrets: inherit`、write permissionは使わない
- `pull_request_target` は使わない
- GitHub API write、merge、auto-merge enable、comment、label、Queue Issue更新は行わない
- eligibleでもDisabled adapterで `write_disabled` になり、`executed=false` を維持する

Issue #41はdry-run executorまでで、Issue #25の実write完了ではありません。
...
- eligibleな場合だけwrite command candidateを作り、既定では `DisabledGitHubWriteAdapter` で `write_disabled` / `executed=false` にする
...
`packages/chatgpt-automation-core/src/auto-merge-executor/` は、auto-merge plan、current-head review evidence、live consumer audit、repository protection audit、CI/check、changed files、write command validationを集約するdry-run executorです。

- 各reportをschema検証し、unknown key、version不一致、repository/PR/SHA不一致、timestamp不正、期限切れ、pagination未完了、API read failureをfail closedにする
- PR #130相当のreview evidenceなしfixtureでは `review_evidence_missing` でwrite commandを生成しない
- eligibleな場合だけ既存github-write converterでcommand候補を作り、既存validatorへ通す
- 本番経路は `DisabledGitHubWriteAdapter` のみで、valid commandでも `write_disabled`、`executed=false`
- audit recordはsanitized summaryだけを出し、plan/report全文、token、Cookie、Authorization、Secret、API response全文を含めない

詳細は [auto-merge-dry-run-executor.md](auto-merge-dry-run-executor.md) を参照します。
...
- reusable workflow refは `REPLACE_WITH_40_CHAR_COMMIT_SHA` を導入時に レビュー済み40桁commit SHAへ置換する
...
- reusable workflow refと `kit-ref` は同じレビュー済み40桁commit SHAへ置換する
...
- `event-payload-json` は `toJson(github.event)` を渡す
- `repository-config-json` とdedupe/cooldown情報は導入先Variableから渡せるが、Secret値は渡さない
- same-run review判定用のrun開始時刻はcallerから渡さず、reusable workflow runtimeが生成する
- reusable workflow refと `kit-ref` は同じレビュー済み40桁commit SHAへ置換する
- Secret、`secrets: inherit`、`runs-on`、`steps`、`run`、`pull_request_target` は使わない
