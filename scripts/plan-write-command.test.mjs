...
test('plan-write-command CLIは通常実行でtrusted actor commandを生成できない', () => {
  ...
  try {
    ...
    assert.equal(output.results[0].validation.blockers[0].reasonCode, 'missing_safety_guard');
    assert.equal(output.results[0].execution.reasonCode, 'missing_safety_guard');
...
test('plan-write-command CLIはfixture限定trust modeでauto-merge planからdisabled write command resultを出力する', () => {
  const dir = mkdtempSync(join(tmpdir(), 'write-command-'));
  try {
    const planPath = join(dir, 'auto-merge-plan.json');
    writeFileSync(planPath, JSON.stringify(autoMergePlan()), 'utf8');
